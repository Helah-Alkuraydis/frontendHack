...
const EscapeRoomBuilder = ({ formData, setFormData, onSubmit, onBack }) => {
  ...
  return (
    // 3. شلنا الخلفية السوداء الثابتة، صار مجرد div عادي يركب داخل أي فورم
    <div className="w-full animate-in fade-in duration-500">
      {/* 4. كود الغرف وأزرار التنقل بين الغرف (activeRoom) ينزل هنا مباشرة */}
      ...
      <div
        className={`rounded-[2.5rem] p-8 border-2 transition-all duration-500 ${roomConfig[activeRoom].bg} ${roomConfig[activeRoom].border}`}
      >
        ...
        {activeRoom === 0 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
            <div className="space-y-2">
              ...
              <input
                className="w-full bg-black/60 border border-pink-500/30 rounded-2xl p-6 text-3xl font-black text-[#ff88cc] text-center tracking-[0.4em] outline-none glitch-text"
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  className="w-full bg-black border border-white/10 rounded-xl p-4 text-emerald-400 font-bold uppercase"
              ...
              <div className="space-y-2">
                ...
                <input
                  className="w-full bg-black border border-white/10 rounded-xl p-4 text-blue-400 text-sm"
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="bg-black/40 border border-blue-500/20 p-6 rounded-[2rem] shadow-inner">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-[12px] text-gray-500 font-mono bg-white/5 px-2 py-1 rounded">
                  PROT_V.2.4
                </span>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter")
                        document.getElementById("addLogBtn").click();
                    }}
                  />
                  <button
                    id="addLogBtn"
                    onClick={() => {
                      ...
                      if (val) {
                        const currentLogs =
                          formData.scenarioData?.rooms?.[1]?.logs || [];
                ...
                <div className="max-h-32 overflow-y-auto space-y-2 bg-black/20 p-3 rounded-xl border border-white/5 custom-scrollbar">
                  {(formData.scenarioData?.rooms?.[1]?.logs || []).length ===
                  0 ? (
                    <p className="text-[9px] text-gray-600 italic text-center py-4">
                      No packet logs deployed yet...
                    </p>
                  ) : (
                    (formData.scenarioData?.rooms?.[1]?.logs || []).map(
                      (log, lIdx) => (
                        <div
                          key={lIdx}
                          className="text-[10px] font-mono flex justify-between items-center bg-white/5 p-2 rounded-lg border border-white/5 group"
                        >
                          ...
                          <button
                            onClick={() => {
                              // نجيب السجلات الحالية
                              const currentLogs =
                                formData.scenarioData?.rooms?.[1]?.logs || [];
                              // نفلتر السجلات ونحذف السجل اللي ضغطنا عليه (حسب الـ Index)
                              const filteredLogs = currentLogs.filter(
                                (_, idx) => idx !== lIdx,
                              );
                              // نحدث الغرفة بالسجلات الجديدة
            ...
            <div className="bg-[#0a1020] border border-blue-500/10 p-6 rounded-[2.5rem] space-y-4 shadow-xl">
              <div className="flex justify-between items-center px-2">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`text-[12px] font-black tracking-tighter px-2 py-1 rounded ${
                      formData.scenarioData?.rooms?.[1]?.options?.length === 5
                        ? "bg-emerald-500/20 text-emerald-500"
                        : "bg-yellow-500/10 text-yellow-500"
                    }`}
                  >
                    {formData.scenarioData?.rooms?.[1]?.options?.length || 0} /
                    5 DEVICES READY
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (ip) {
                      const opts =
                        formData.scenarioData?.rooms?.[1]?.options || [];
                      if (opts.length < 5) {
              ...
              <div className="grid grid-cols-5 gap-3 h-32">
                {/* عرض الأجهزة المضافة */}
                {(formData.scenarioData?.rooms?.[1]?.options || []).map(
                  (opt, optIdx) => (
                    <div
                      key={optIdx}
                    ...
                    >
                      <div
                        className={`text-2xl mb-1 transition-transform group-hover:scale-110 ${opt === formData.scenarioData?.rooms?.[1]?.answer ? "animate-pulse" : ""}`}
                      >
                        {opt === formData.scenarioData?.rooms?.[1]?.answer
                          ? "☣️"
                          : "💻"}
                      </div>
                      <p className="text-[8px] text-gray-500 uppercase font-black tracking-tighter mb-1">
                        Device_{optIdx + 1}
                      </p>
                      <p className="font-mono text-[9px] text-blue-400 truncate w-full text-center">
                        {opt}
                      </p>
                      ...
                      {/* <button
                        onClick={(e) => {
                          e.stopPropagation();
                          const filtered =
                            formData.scenarioData.rooms[1].options.filter(
                              (_, i) => i !== optIdx,
                            );
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation(); // عشان ما يحدد الجهاز كإجابة لما تضغطين زر الحذف
                          ...
                          // إذا الجهاز اللي حذفناه كان هو "الإجابة الصحيحة"، لازم نصفر الإجابة بعد
                          if (
                            opt === formData.scenarioData?.rooms?.[1]?.answer
                          ) {
                            updates.answer = "";
                ...
                {[
                  ...
                  <div
                    key={`empty-${i}`}
                    className="border-2 border-dashed border-white/5 bg-black/20 rounded-2xl flex flex-col items-center justify-center opacity-20 group"
                  >
                    <div className="text-xl mb-1 text-gray-600">📡</div>
                    <p className="text-[7px] font-black text-gray-600">
                      OFFLINE
                    </p>
                  </div>
                ))}
              ...
              <div className="bg-black/40 p-3 rounded-xl border border-white/5">
                <p className="text-[11px] text-center text-gray-500 italic flex items-center justify-center gap-2">
                  ...
                  PROTOCOL: Analyze the logs above, then click the suspicious
                  device to set it as the{" "}
        ...
        {activeRoom === 2 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 text-center">
            <div className="bg-[#0a1020] border-l-4 border-emerald-500 p-6 rounded-2xl text-left">
              <label className="text-[10px] text-emerald-500 font-black uppercase mb-2 block">
                Boolean Equation
              </label>
              <textarea
                className="w-full bg-transparent text-lg font-mono text-white outline-none h-20"
            ...
            <div className="grid grid-cols-2 gap-6 pt-4">
              <button
                ...
                className={`p-8 rounded-[2.5rem] border-2 transition-all ${formData.scenarioData?.rooms?.[2]?.answer === "TRUE" ? "bg-emerald-500/20 border-emerald-500" : "bg-black/40 border-white/5 opacity-40"}`}
              >
              ...
              <button
                ...
                className={`p-8 rounded-[2.5rem] border-2 transition-all ${formData.scenarioData?.rooms?.[2]?.answer === "FALSE" ? "bg-rose-500/20 border-rose-500" : "bg-black/40 border-white/5 opacity-40"}`}
              >
                <span className="text-2xl font-black text-rose-500">FALSE</span>
              </button>
            </div>
          </div>
        )}
        ...
        {activeRoom === 3 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="p-6 bg-[#0a0f1d] rounded-[2.5rem] border-2 border-white/5 text-left">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-3">
                  <label className="text-[10px] text-gray-500 uppercase font-black italic">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-6 bg-pink-500/5 border-2 border-pink-500/20 rounded-[2.5rem] text-left relative overflow-hidden">
                <div className="absolute top-2 right-4 text-[7px] font-black text-pink-500/30 uppercase italic">
                  Layer_01: First Sight
                </div>
                <label className="text-[12px] text-pink-400 font-black uppercase mb-3 block">
                ...
                <div className="bg-black/60 rounded-xl p-4 font-mono text-xs text-[#ff88cc] break-all min-h-[60px] flex items-center justify-center border border-pink-500/10">
                  {formData.scenarioData?.rooms?.[3]?.room4?.layer1 ||
                    "WAITING_FOR_DATA..."}
                </div>
              </div>

              <div className="p-6 bg-purple-500/5 border-2 border-purple-500/20 rounded-[2.5rem] text-left relative overflow-hidden">
                <div className="absolute top-2 right-4 text-[7px] font-black text-purple-500/30 uppercase italic">
                  Layer_02: Decoded Cipher
                </div>
                <label className="text-[12px] text-purple-400 font-black uppercase mb-3 block">
        ...
        <button
          onClick={() => {
            ...
            if (activeRoom === 0) {
              // Cipher
              if (!room?.puzzle_data)
                missingFields.push("Cipher Text (The code)");
              if (!room?.answer)
                missingFields.push("Clean Answer (Decrypted text)");
              if (!room?.hint) missingFields.push("Cipher Hint (Shift value)");
            } else if (activeRoom === 1) {
              // Network
              if (!room?.logs?.length)
                missingFields.push("Network Logs (At least 1)");
              if (room?.options?.length !== 5)
                missingFields.push(
                  `Exactly 5 Devices (Current: ${room?.options?.length || 0})`,
                );
              if (!room?.answer)
                missingFields.push("Infected Target (Click a device)");
            } else if (activeRoom === 2) {
              // Logic
              if (!room?.puzzle_data) missingFields.push("Boolean Equation");
              if (!room?.answer)
                missingFields.push("Logic Result (TRUE/FALSE)");
