        ...
        html: `
          <div class="flex flex-col items-center gap-6">
            ...
            <div class="space-y-2">
                ...
                <p class="text-gray-500 text-[10px] font-mono tracking-tighter uppercase italic">
                </p>
            </div>
          </div>
        `,
        background: '#050810',
        confirmButtonText: 'FINICH THE GAME',
        confirmButtonColor: '#00ff96',
...
socket.on("escape_game_over", (data) => {
    Swal.fire({
        ...
        html: `
            <div className="text-center space-y-4">
                ...
                <p className="text-red-500 font-black uppercase tracking-widest">${data.msg}</p>
  ...
  return (
    <div className="h-screen bg-[#050810] flex relative overflow-hidden font-mono text-white">
          ...
          <header className="flex items-center justify-between px-8 py-4 bg-[#080c16] border-b border-[#00ff9610]">
            <div className="flex items-center gap-6">
              <button 
                ...
                className="p-2 bg-white/5 hover:bg-red-500/20 border border-white/10 rounded-full transition-all text-gray-500 hover:text-red-500 group"
                title="Abort Mission"
              >
      <ArrowLeft size={20} className="group-hover:-translate-x-1 transition-transform" />
    </button>
              <div className="flex flex-col text-left">
            <span className="text-[10px] text-[#00ff96] font-black tracking-[0.3em] uppercase">⬡ Mission_Active</span>
            <h2 className="text-lg font-black italic uppercase">Room_0{roomIdx + 1}: {roomData?.title || "Initializing..."}</h2>
          </div>
          </div>
          
          <div className="flex items-center gap-5">
            <div className="text-right">
                <p className="text-[10px] text-gray-500 uppercase font-black">Agent_Specialty</p>
                  <p className="text-cyan-400 text-s font-black italic">{currentRole}</p>
             </div>
             <div className="bg-[#0d1a0d] border border-[#00ff9630] px-5 py-1 rounded-sm">
      ...
      {!isChatOpen && (
        <button 
          ...
          className="fixed bottom-8 right-8 z-[1000] p-5 bg-[#00ff96] text-black rounded-2xl shadow-[0_0_30px_rgba(0,255,150,0.4)] hover:scale-110 transition-all border border-white/20"
        >
          <div className="relative">
            <MessageSquareCode size={28} />
            {unreadCount > 0 && (
              <span className="absolute -top-6 -right-6 bg-red-600 text-white text-[10px] font-black w-6 h-6 flex items-center justify-center rounded-full animate-bounce border-2 border-[#050810]">
                {unreadCount}
              </span>
            )}
          </div>
      ...
      <div className={`fixed top-0 right-0 h-full z-[1001] transition-transform duration-500 ease-in-out transform ${isChatOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <button onClick={() => setIsChatOpen(false)} className="absolute top-4 right-4 z-[1002] p-2 text-gray-500 hover:text-white">
          <X size={20} />
        </button>
