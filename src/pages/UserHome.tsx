...
if (dashboardRes.data.latestAchievement) {
  ...
  return (
    <MainLayout 
      ...
      headerActions={
        <button 
          ...
          className="text-[10px] font-black uppercase tracking-[0.2em] text-white border border-white/20 px-5 py-2.5 rounded-xl transition-all hover:bg-white/10 active:scale-95 shadow-[0_0_15px_rgba(255,255,255,0.05)]"
        >
          Replay Tour
        </button>
      }
    >
      ...
      {/* Banner Section */}
      <div className="w-full relative bg-[#121620]/80 backdrop-blur-md rounded-[2.5rem] p-12 mb-10 flex justify-between items-center border border-gray-800/50 shadow-xl overflow-hidden min-h-[320px]">
          <div className="hidden lg:block w-64 h-72 bg-contain bg-no-repeat bg-center absolute left-12 -bottom-6 drop-shadow-2xl transition-transform hover:scale-105 duration-500" 
               style={{backgroundImage: `url('${characterImg}')`}}>
          </div>
          <div className="z-10 relative ml-auto text-right pr-4">
             <p className="text-gray-400 text-sm mb-4 font-medium tracking-widest uppercase italic">{new Date().toDateString()}</p>
             <h1 className="text-4xl md:text-5xl font-black mb-3 text-white italic tracking-tight">{greeting}</h1>
      ...
      <h3 className="text-2xl font-black mb-8 tracking-tighter italic uppercase text-white/90">Most popular games</h3>
      ...
      <h3 className="text-2xl font-black mb-8 tracking-tighter italic uppercase text-white/90">Recent Activity</h3>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pb-12">
        
        <div className="bg-[#121620]/80 backdrop-blur-md p-8 rounded-[2.5rem] border border-gray-800/50 flex flex-col items-center justify-center text-center shadow-xl min-h-[280px] group hover:border-[#ff3b6b]/30 transition-all">
    {lastActivity ? (
        <>
       ...
       <div className="bg-[#121620]/80 backdrop-blur-md p-8 rounded-[2.5rem] border border-gray-800/50 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-xl min-h-[280px]">
    ...
    {latestAchievement ? (
       ...
       <div className="bg-[#121620]/80 backdrop-blur-md p-8 rounded-[2.5rem] border border-gray-800/50 flex flex-col items-center justify-center text-center shadow-xl min-h-[280px] relative overflow-hidden">
    <div className="absolute inset-0 bg-blue-600/5 blur-[40px] rounded-full"></div>
...
const GameCard = ({ title, desc, gradient, borderColor, btnColor, icon, bgImage }: any) => (
  <div className={`relative overflow-hidden rounded-[3rem] ${gradient} backdrop-blur-md border ${borderColor} p-10 flex flex-col justify-between min-h-[300px] group transition-all duration-500 hover:scale-[1.02] shadow-2xl`}>
